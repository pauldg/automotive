...
class GenreFeatureData():
    ...
    genre_list = ['new_filter','old_filter']
    data_type = 'data_mazda_2000ms'

    INPUT_DIR = "../../../01_Development/02_Data/02_Transformation/01_etl/"
    dir_trainfolder = INPUT_DIR + data_type + "/_train"
    dir_devfolder = INPUT_DIR + data_type + "/_validation"
    dir_testfolder = INPUT_DIR + data_type + "/_test"
    dir_all_files = INPUT_DIR + data_type
    ...
    new_X_preprocessed_data = 'data_new_input.npy'
    ...
    def __init__(self):
        self.hop_length = 200  # 512
        self.timeseries_length_list = []
        
        
    def load_new_data(self, newfiles_list):

        #self.precompute_min_timeseries_len(all_files_list)
        print("[DEBUG] total number of files: " + str(len(newfiles_list)))
        print(newfiles_list)
        # Training set
        new_X = self.extract_audio_features(newfiles_list)[0]
        
#         print(new_X)
        
        new_X = new_X.swapaxes(1,2).reshape(new_X.shape[0],-1)

        with open(self.new_X_preprocessed_data, 'wb') as f:
            np.save(f, new_X)
    ...
    def load_preprocess_data(self):
        ...
        self.train_X = self.train_X.swapaxes(1,2).reshape(self.train_X.shape[0],-1)
#         self.train_Y = self.train_Y.swapaxes(1,2).reshape(self.train_Y.shape[0],-1)
        ...
        self.dev_X = self.dev_X.swapaxes(1,2).reshape(self.dev_X.shape[0],-1)
#         self.dev_Y = self.dev_Y.swapaxes(1,2).reshape(self.dev_Y.shape[0],-1)
        ...
        self.test_X = self.test_X.swapaxes(1,2).reshape(self.test_X.shape[0],-1)
#         self.test_Y = self.test_Y.swapaxes(1,2).reshape(self.test_Y.shape[0],-1)
    ...
    def extract_audio_features(self, list_of_audiofiles):
        timeseries_length = 481
        data = np.zeros((len(list_of_audiofiles), timeseries_length, 30))
        target = []
        ...
        for i, file in enumerate(list_of_audiofiles):
            ...
            mfcc = librosa.feature.mfcc(y=y, sr=sr, hop_length=self.hop_length, n_mfcc=20)
#             spectral_center = librosa.feature.spectral_centroid(y=y, sr=sr, hop_length=self.hop_length)
#             stft = librosa.stft(y=y, hop_length=genre_features.hop_length,n_fft=60)
#             chroma = librosa.feature.chroma_stft(y=y, sr=sr, hop_length=self.hop_length)
#             spectral_contrast = librosa.feature.spectral_contrast(y=y, sr=sr, hop_length=self.hop_length)

            splits = re.split('[ /]', file)[-1]
#             print(splits)
            genre = re.split('[ .]', splits)[0]
#             print(genre)
            target.append(genre)

            data[i, :, 0:20] = mfcc.T[0:timeseries_length, :]
#             data[i, :, 20:30] = spectral_center.T[0:timeseries_length, :]
#             data[i, :, 20:30] = stft.T[0:timeseries_length, 10:20]
#             data[i, :, 26:33] = spectral_contrast.T[0:timeseries_length, :]
#             data[i, :, 33:41] = stft.T[0:timeseries_length, 24000:24008]
